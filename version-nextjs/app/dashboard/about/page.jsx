  ...
  useEffect(() => {
    ...
    fetch('/api/images')
      ...
      .then(data => {
        ...
        if (profileImage) {
          setPhoto(`data:${profileImage.type};base64,${profileImage.base64}`);
        }
      });
  }, []);
  ...
  useEffect(() => {
    if (student && mapRef.current && !mapInstanceRef.current) {
      import('leaflet').then(L => {
        ...
        L.default.marker([student.latitude, student.longitude])
          .addTo(map)
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-bg rounded-lg p-4 text-center">
                  <p className="text-xs text-muted mb-1">Note 1</p>
                  <p className="text-xl font-bold text-primary">{student.note1}/20</p>
                </div>
                <div className="bg-bg rounded-lg p-4 text-center">
                  <p className="text-xs text-muted mb-1">Note 2</p>
                  <p className="text-xl font-bold text-primary">{student.note2}/20</p>
                </div>
                <div className="bg-bg rounded-lg p-4 text-center">
                  <p className="text-xs text-muted mb-1">Moyenne</p>
                  <p className="text-xl font-bold text-primary">{student.moyenne.toFixed(1)}/20</p>
                  <span className={`inline-block mt-1 text-xs font-semibold px-2.5 py-0.5 rounded-full ${badge.bg}`}>
                    {badge.label}
                  </span>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['HTML', 'CSS', 'JavaScript', 'PHP', 'MySQL', 'Next.js', 'React', 'TypeScript', 'Tailwind CSS', 'Laravel'].map(skill => (
                  <span key={skill} className="bg-secondary/10 text-secondary text-sm font-medium px-3 py-1 rounded-full">
                    {skill}
                  </span>
                ))}
              </div>
