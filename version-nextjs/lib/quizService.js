 ...
 */
export function getQuizQuestions(quizNumber) {
  const quiz = QUIZ_DATA[quizNumber];
  if (!quiz) return null;
 ...
 */
export function calculateQuizScore(quizNumber, studentAnswers) {
  const quiz = QUIZ_DATA[quizNumber];
  ...
  const score = Math.round((correct / quiz.questions.length) * 20);
  return {
    correct,
    total: quiz.questions.length,
    score: Math.min(score, 20), // Clamp à 20
  };
}
